...
import {
  listMaintenanceSecurityReducer,
  initialStateListMaintenanceSecurity,
} from "src/reducers/securityReducer";

import {
  deleteMaintenanceSecurity,
...
import { SecurityMaintenance } from "@/types/securityTypes";
...
export default function ListMaintenanceSecurity() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenanceSecurity(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<SecurityMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/security?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenanceSecurity(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Security`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/maintenance/add/security`)
            }
