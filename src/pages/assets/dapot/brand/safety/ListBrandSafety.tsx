...
import {
  listBrandSafetyReducer,
  initialStateListBrandSafety,
} from "src/reducers/safetyReducer";

import { deleteSafety, getBrandSafety } from "@/services/safety/dapotSafety";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
import { SafetyBrand } from "@/types/safetyTypes";
...
export default function ListBrandSafety() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandSafety(page, dispatch, globalFilter, nopage);
  ...
  const columns: ColumnDef<SafetyBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/safety?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteSafety(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Safety`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/safety`)
            }
