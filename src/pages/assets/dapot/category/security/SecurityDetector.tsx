...
import {
  ...
  exportDetectorsXlsx,
  getDetectors,
} from "@/services/security/dapotDetector";
import {
  initialStateListOsc,
  listOscReducer,
} from "src/reducers/securityReducer";
import { oScSecurity } from "@/types/securityTypes";
export default function SecurityDetector() {
  ...
  useEffect(() => {
    const fetchDetectors = async () => {
      try {
        ...
        const data = await getDetectors(page, dispatch, globalFilter, nopage);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_OSCS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch detector",
        });
      }
    };
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/security/detector?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/security/detector?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteDetector(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportDetectorsCsv}
            exportXlsx={exportDetectorsXlsx}
          />
