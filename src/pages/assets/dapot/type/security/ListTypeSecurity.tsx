...
import {
  initialStateListTypeSecurity,
  listTypeSecurityReducer,
...
export default function ListTypeSecurity() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        const data = await getTypeSecurity(
          page,
          dispatch,
          globalFilter,
          nopage
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/type/update/security?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTypeSecurity(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Type Security`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/type/add/security`)
            }
