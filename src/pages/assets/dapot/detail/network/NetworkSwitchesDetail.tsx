...
import { getNetworkswitche } from "@/services/network/dapotNetworkSwitches";

export default function NetworkSwitchesDetail() {
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getNetworkswitche(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_OTHERSC",
          payload: {
        ...
        <>
          ...
          <div className={styles.detailSubCategory}>
            ...
            <CardDetail
              title="Foto"
              type="foto"
              urlImage="https://192.168.1.62:2001/images/network"
              imageTitle1={photo1}
              imageTitle2={photo2}
              imageTitle3={photo3}
            />
