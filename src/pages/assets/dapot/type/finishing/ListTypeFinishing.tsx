...
import {
  initialStateListTypeFinishingBuilding,
  listTypeFinishingBuildingReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  deleteTypeFinishingbuilding,
  getTypeFinishingbuilding,
} from "@/services/finishing_building/dapotFinishingBuildings";
import { FinishingBuildingType } from "@/types/finishingBuildingTypes";

export default function ListTypeFinishing() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        const data = await getTypeFinishingbuilding(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<FinishingBuildingType>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/type/update/buildingfinishing?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTypeFinishingbuilding(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Type Building Finishing`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/type/add/buildingfinishing`)
            }
