...
import { getTrafo, updateTrafo } from "@/services/electrical/dapotTrafo";
...
export default function ElectricalTrafoUpdate() {
  const navigate = useNavigate();
  const [searchParams, _] = useSearchParams();
  const [state, dispatch] = useReducer(updateTrafoReducer, initialTrafoState);
  const {
    asset_id,
    ne_id,
    site_id,
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "ESC007");
    ...
    formData.append("transform_ratio", transform_ratio);
    ...
    const postnew = async (data: any) => {
      const result = await updateTrafo(
        data,
        dispatch,
        searchParams.get("id") || "",
        asset_id
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/electrical/trafo?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getTrafo(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_TRAFO",
          payload: {
            ...
            transform_ratio: data.transform_ratio,
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Transform Ratio</p>
                  <input
                    type="number"
                    className={styles.inputAsset}
