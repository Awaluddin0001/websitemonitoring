...
import {
  listMaintenancePumpReducer,
  initialStateListMaintenancePump,
} from "src/reducers/pumpReducer";

import {
  deleteMaintenancePump,
  getMaintenancePump,
...
import { PumpMaintenance } from "@/types/pumpTypes";
...
export default function ListMaintenancePump() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenancePump(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<PumpMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/pump?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenancePump(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Pump`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/maintenance/add/pump`)
            }
