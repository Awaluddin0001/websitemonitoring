...
import { Trafo } from "@/types/electricalTypes";
import {
  deleteTrafo,
  exportTrafosCsv,
  exportTrafosXlsx,
  getTrafos,
} from "@/services/electrical/dapotTrafo";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ElectricalTrafo() {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const [state, dispatch] = useReducer(listTrafoReducer, initialStateListTrafo);
  const {
    trafos,
    pagination,
    isLoading,
  ...
  useEffect(() => {
    const fetchRectifiers = async () => {
      try {
        ...
        const data = await getTrafos(page, dispatch, globalFilter, nopage);
        console.log(data);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_TRAFOS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
  ...
  const columns: ColumnDef<Trafo>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Trafo Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/electrical/trafo?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "manufactur", header: "Manufaktur" },
        { accessorKey: "transform_ratio", header: "Ratio Trafo" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/electrical/trafo?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTrafo(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportTrafosCsv}
            exportXlsx={exportTrafosXlsx}
          />
                            ...
                            <td
                              ...
                              style={{
                                ...
                                borderBottomLeftRadius:
                                  row.index === trafos.length - 1 && ind === 0
                                    ? "10px"
                                    : undefined,
                                borderBottomRightRadius:
                                  row.index === trafos.length - 1 &&
