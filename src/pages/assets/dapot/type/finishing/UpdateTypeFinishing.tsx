...
import {
  initialStateUpdateFinishingBuildingType,
  updateFinishingBuildingTypeReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  getOneTypeFinishingbuilding,
  getSubCategoriesFinishingbuilding,
  updateTypeFinishingbuilding,
...
export default function UpdateTypeFinishing() {
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      try {
        const data = await getSubCategoriesFinishingbuilding();
    ...
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneTypeFinishingbuilding(
          dispatch,
          searchParams.get("id")
        );
        console.log(result);
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateTypeFinishingbuilding(
        name,
      ...
      if (result.success) {
        navigate(`/main/assets/datapotensi/type/list/buildingfinishing?page=1`);
      }
    };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Type Building Finishing`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Update Data Type Untuk Building Finishing`} />
