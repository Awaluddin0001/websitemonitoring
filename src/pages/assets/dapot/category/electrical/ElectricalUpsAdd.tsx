...
import { postNewUps } from "@/services/electrical/dapotUps";
interface Options {
  value: string;
  label: string;
}
...
export default function ElectricalUpsAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await postNewUps(data, dispatch);
      if (result.success) {
        navigate(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Ups`} />
