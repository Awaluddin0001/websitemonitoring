...
import {
  initialStateUpdateFinishingBuildingMaintenance,
  updateFinishingBuildingMaintenanceReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  getOneMaintenanceFinishingbuilding,
  updateMaintenanceFinishingbuilding,
} from "@/services/finishing_building/dapotFinishingBuildings";
export default function UpdateMaintenanceFinishing() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceFinishingbuilding(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/maintenance/list/buildingfinishing?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceFinishingbuilding(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Building Finishing`} />
