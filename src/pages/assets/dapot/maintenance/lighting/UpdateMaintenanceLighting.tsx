...
import {
  getOneMaintenanceLighting,
  updateMaintenanceLighting,
} from "@/services/lighting/dapotLighting";
import React, { useEffect, useReducer } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  initialStateUpdateLightMaintenance,
  updateLightMaintenanceReducer,
} from "src/reducers/lightingReducer";
export default function UpdateMaintenanceLighting() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceLighting(data, dispatch);
      if (result.success) {
        navigate(`/main/assets/datapotensi/maintenance/list/lighting?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceLighting(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Lighting`} />
