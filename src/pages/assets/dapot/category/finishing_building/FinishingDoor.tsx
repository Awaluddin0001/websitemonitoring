...
import {
  ...
  exportDoorsXlsx,
  getDoors,
} from "@/services/finishing_building/dapotFinishingDoor";
import { finishingListButtons } from "@/routes/dapotCategory";
export default function FinishingDoor() {
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/buildingfinishing/door?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/buildingfinishing/door?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteDoor(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportDoorsCsv}
            exportXlsx={exportDoorsXlsx}
          />
