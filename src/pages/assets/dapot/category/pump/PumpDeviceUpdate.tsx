...
export default function PumpDeviceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await updatePumpdevice(data, dispatch, asset_id, asset_id);
      console.log(result);
      if (result.success) {
        navigate(`/main/assets/datapotensi/category/list/pump/pump?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getPumpdevice(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_DEVICE",
          payload: {
    ...
    fetchBrand(getBrandPump, dispatch);
    fetchVendor(getVendorPump, dispatch);
    fetchMaintenance(getMaintenancePump, dispatch);
  }, []);
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Water_flow</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={water_flow || ""}
