...
import {
  initialStateListVideorecording,
  listVideorecordingReducer,
} from "src/reducers/securityReducer";
import { VideoRecording } from "@/types/securityTypes";
import {
  deleteVideorecording,
  exportVideorecordingsCsv,
  exportVideorecordingsXlsx,
  getVideorecordings,
} from "@/services/security/dapotVideoRecording";
export default function SecurityVideoRecording() {
  ...
  useEffect(() => {
    const fetchVideorecordings = async () => {
      try {
        ...
        dispatch({ type: "SET_VIDEORECORDINGS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch videorecording",
        });
      }
    };
  ...
  const columns: ColumnDef<VideoRecording>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Videorecording Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/security/video_recording?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/security/video_recording?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVideorecording(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportVideorecordingsCsv}
            exportXlsx={exportVideorecordingsXlsx}
          />
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index === videorecordings.length - 1 &&
                                    ind === 0
                                      ? "10px"
                                      : undefined,
