...
import {
  getOneMaintenancePump,
  updateMaintenancePump,
...
export default function UpdateMaintenancePump() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenancePump(data, dispatch);
      if (result.success) {
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenancePump(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Pump`} />
