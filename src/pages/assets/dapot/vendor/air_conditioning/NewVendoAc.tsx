...
import {
  initialStateUpdateAirconditioningVendor,
  updateAirconditioningVendorReducer,
} from "src/reducers/airConditioningReducer";
import { postVendorAirconditioning } from "@/services/air_conditioning/dapotAirConditioning";
export default function NewVendorAirConditioning() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await postVendorAirconditioning(
        company,
        company_user_name,
        number_phone,
        dispatch
      ...
      if (result.success) {
        navigate(`/main/assets/datapotensi/vendor/list/airconditioning?page=1`);
      }
    };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Vendor air conditioning`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Input Data Vendor Untuk air conditioning`} />
