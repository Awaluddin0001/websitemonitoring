...
import { postNewDoor } from "@/services/finishing_building/dapotFinishingDoor";
...
export default function FinishingDoorAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "BSC03");
    ...
    const postnew = async (data: any) => {
      const result = await postNewDoor(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/buildingfinishing/door?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeFinishingbuilding, dispatch, "BSC03");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk door`} />
