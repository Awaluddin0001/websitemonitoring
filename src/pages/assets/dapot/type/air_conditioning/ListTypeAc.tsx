...
import {
  initialStateListTypeAirconditioning,
  listTypeAirconditioningReducer,
} from "src/reducers/airConditioningReducer";
import {
  deleteTypeAirconditioning,
  getTypeAirconditioning,
} from "@/services/air_conditioning/dapotAirConditioning";
import { AirconditioningType } from "@/types/airConditioningTypes";

export default function ListTypeAirconditioning() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        const data = await getTypeAirconditioning(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<AirconditioningType>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/type/update/airconditioning?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTypeAirconditioning(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Type Air Conditioning`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/type/add/airconditioning`)
            }
