...
import {
  initialStateListCeramic,
  listCeramicReducer,
} from "src/reducers/finishingBuildingReducer";
import { Ceramic } from "@/types/finishingBuildingTypes";
import {
  deleteCeramic,
  exportCeramicsCsv,
  exportCeramicsXlsx,
  getCeramics,
} from "@/services/finishing_building/dapotCeramic";
import { finishingListButtons } from "@/routes/dapotCategory";
export default function FinishingCeramic() {
  ...
  useEffect(() => {
    const fetchCeramics = async () => {
      try {
        ...
        const data = await getCeramics(page, dispatch, globalFilter, nopage);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_FINISHINGBUILDINGCERAMICS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch ceramics",
        });
      }
    };
  ...
  const columns: ColumnDef<Ceramic>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Ceramic Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/buildingfinishing/ceramic?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/buildingfinishing/ceramic?id=${row.original.id}`
                  )
                }
              >
  ...
  const table = useReactTable({
    data: finishingbuildingceramics,
    columns,
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteCeramic(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportCeramicsCsv}
            exportXlsx={exportCeramicsXlsx}
          />
          <div className={styles.tableSeal}>
            <div className={styles.tableWrapper}>
              <DapotButtonsCategory
                ...
                subCategory="buildingfinishing"
              />
              {finishingbuildingceramics.length > 0 ? (
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index ===
                                      finishingbuildingceramics.length - 1 &&
                                    ind === 0
                                      ? "10px"
                                      : undefined,
