...
import { postNewTrafo } from "@/services/electrical/dapotTrafo";
interface Options {
  value: string;
  label: string;
}
...
export default function ElectricalTrafoAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await postNewTrafo(data, dispatch);
      if (result.success) {
        navigate(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Trafo`} />
