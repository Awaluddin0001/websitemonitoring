...
import { postNewVideorecording } from "@/services/security/dapotVideoRecording";
...
export default function SecurityVideoRecordingAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "SSC07");
    ...
    const postnew = async (data: any) => {
      const result = await postNewVideorecording(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/security/video_recording?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeSecurity, dispatch, "SSC07");
  }, []);
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Video Recording`} />
