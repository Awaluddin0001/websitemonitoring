...
import { postVendorExtinguish } from "@/services/extinguish/dapotExtinguish";
import React, { useReducer } from "react";
import { useNavigate } from "react-router-dom";
import {
  initialStateUpdateExtinguishVendor,
  updateExtinguishVendorReducer,
} from "src/reducers/extinguishReducer";
export default function NewVendorExtinguish() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await postVendorExtinguish(
        company,
        company_user_name,
        number_phone,
        dispatch
      ...
      if (result.success) {
        navigate(`/main/assets/datapotensi/vendor/list/extinguish?page=1`);
      }
    };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Vendor Extinguish`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Input Data Vendor Untuk Extinguish`} />
