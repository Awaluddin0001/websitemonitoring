...
import { getAlarm, updateAlarm } from "@/services/security/dapotAlarm";
...
export default function SecurityAlarmUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "SSC02");
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/security/alarm?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getCctvs = async () => {
      try {
        const data = await getAlarm(searchParams.get("id"), dispatch);
        console.log(data);
        dispatch({
          type: "GET_OSC",
          payload: {
    ...
    fetchType(getTypeSecurity, dispatch, "SSC02");
  }, []);
