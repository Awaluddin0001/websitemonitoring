...
import { postNewMebel } from "@/services/finishing_building/dapotMebel";
...
export default function FinishingMebelAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "BSC04");
    ...
    const postnew = async (data: any) => {
      const result = await postNewMebel(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/buildingfinishing/mebel?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeFinishingbuilding, dispatch, "BSC04");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk mebel`} />
