...
import { SecurityVendor } from "@/types/securityTypes";
import { renderPagination } from "@/components/table/RenderPagination";
import {
  initialStateListVendorSecurity,
  listVendorSecurityReducer,
} from "src/reducers/securityReducer";

export default function ListVendorSecurity() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorSecurity(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<SecurityVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/security?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorSecurity(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
    ...
    <>
      {isLoading ? (
        ...
        <>
          ...
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/vendor/add/security`)
            }
