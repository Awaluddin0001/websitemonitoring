...
import { postNewFirewall } from "@/services/network/dapotFirewalls";
...
export default function NetworkFirewallAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "NSC02");
    ...
    const postnew = async (data: any) => {
      const result = await postNewFirewall(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/network/firewalls?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeNetwork, dispatch, "NSC02");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Firewall`} />
