...
export default function FluidDeviceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateFluiddevice(
        data,
        dispatch,
        asset_id,
        asset_id
      ...
      console.log(result);
      if (result.success) {
        navigate(`/main/assets/datapotensi/category/list/fluid/fluid?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getFluiddevice(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_DEVICE",
          payload: {
    ...
    fetchBrand(getBrandFluid, dispatch);
    fetchVendor(getVendorFluid, dispatch);
    fetchMaintenance(getMaintenanceFluid, dispatch);
  }, []);
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Fluid</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={fluid || ""}
