...
export default function UpdateTypeNetwork() {
  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneTypeNetwork(
          dispatch,
          searchParams.get("id")
        );
        console.log(result);
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateTypeNetwork(
        name,
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Type Network`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Update Data Type Untuk Network`} />
