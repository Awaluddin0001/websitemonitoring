...
import { Storage } from "@/types/networkTypes";
import {
  deleteStorage,
  exportStoragesCsv,
  exportStoragesXlsx,
  getStorages,
} from "@/services/network/dapotStorage";
export default function NetworkStorage() {
  ...
  useEffect(() => {
    const fetchStorages = async () => {
      try {
        ...
        const data = await getStorages(page, dispatch, globalFilter, nopage);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_STORAGES", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch storages",
        });
      }
    };
  ...
  const columns: ColumnDef<Storage>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Storage Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/network/storage?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/network/storage?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteStorage(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportStoragesCsv}
            exportXlsx={exportStoragesXlsx}
          />
