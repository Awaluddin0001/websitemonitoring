...
import { postNewAccesscontrol } from "@/services/security/dapotAccessControl";
...
export default function SecurityAccessControlAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "SSC01");
    ...
    const postnew = async (data: any) => {
      const result = await postNewAccesscontrol(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/security/access_control?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeSecurity, dispatch, "SSC01");
  }, []);
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Access Control`} />
