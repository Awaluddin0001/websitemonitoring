...
import {
  initialStateListVendorAirconditioning,
  listVendorAirconditioningReducer,
} from "src/reducers/airConditioningReducer";
import {
  deleteVendorAirconditioning,
  getVendorAirconditioning,
} from "@/services/air_conditioning/dapotAirConditioning";
import { AirconditioningVendor } from "@/types/airConditioningTypes";

export default function ListVendorAirConditioning() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorAirconditioning(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<AirconditioningVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/airconditioning?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorAirconditioning(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
