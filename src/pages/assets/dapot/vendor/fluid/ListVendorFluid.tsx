...
import {
  initialStateListVendorFluid,
  listVendorFluidReducer,
} from "src/reducers/fluidReducer";
import { deleteVendorFluid, getVendorFluid } from "@/services/fluid/dapotFluid";
import { FluidVendor } from "@/types/fluidTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorFluid() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorFluid(page, dispatch, globalFilter, nopage);
  ...
  const columns: ColumnDef<FluidVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/fluid?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorFluid(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Vendor Fluid `} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/vendor/add/fluid`)
            }
