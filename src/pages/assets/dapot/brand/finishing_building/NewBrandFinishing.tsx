...
import { postBrandFinishingbuilding } from "@/services/finishing_building/dapotFinishingBuildings";
export default function NewBrandFinishing() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (name: any) => {
      const result = await postBrandFinishingbuilding(name, dispatch);
      if (result.success) {
        navigate(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Brand Building Finishing`} />
