...
export default function SafetyDeviceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateSafetydevice(
        data,
        dispatch,
        searchParams.get("id")!,
        asset_id
      ...
      console.log(result);
      if (result.success) {
        navigate(`/main/assets/datapotensi/category/list/safety/safety?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getSafetydevice(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_DEVICE",
          payload: {
    ...
    fetchBrand(getBrandSafety, dispatch);
    fetchVendor(getVendorSafety, dispatch);
  }, []);
