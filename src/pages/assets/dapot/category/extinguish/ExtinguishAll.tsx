...
import { getExtinguishs } from "@/services/extinguish/dapotExtinguish";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
import { Extinguish } from "@/types/extinguishTypes";

export default function ExtinguishAll() {
  ...
  useEffect(() => {
    const fetchExtinguishs = async () => {
      try {
        const page = searchParams.get("page") || "1";
        const nopage = globalFilter ? "no" : undefined;
        const data = await getExtinguishs(page, dispatch, globalFilter, nopage);
        ...
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch extinguish",
        });
      }
    };
  ...
  const columns: ColumnDef<Extinguish>[] = [
    { accessorKey: "asset_id", header: "Extinguish ID" },
    ...
    { accessorKey: "room_name", header: "Ruangan" },
    {
      ...
      cell: ({ row }) => (
        <Link
          to={`/main/assets/datapotensi/detail/extinguish/extinguish?id=${row.original.device_id}`}
          style={{
            color: "#000",
            fontSize: "1.8rem",
          }}
