...
export default function FurnitureDeviceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateFurnituredevice(
        data,
        dispatch,
        searchParams.get("id")!,
        asset_id
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/furniture/furniture?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getFurnituredevice(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_DEVICE",
          payload: {
    ...
    fetchBrand(getBrandFurniture, dispatch);
    fetchVendor(getVendorFurniture, dispatch);
  }, []);
