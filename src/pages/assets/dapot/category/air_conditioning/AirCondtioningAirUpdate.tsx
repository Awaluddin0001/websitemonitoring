...
import { getAir, updateAir } from "@/services/air_conditioning/dapotAir";
import {
...
export default function AirConditionongAirUpdate() {
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getAir(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_AIRDEVICE",
          payload: {
