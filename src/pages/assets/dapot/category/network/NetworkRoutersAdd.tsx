...
import { postNewRouter } from "@/services/network/dapotRouters";
...
export default function NetworkRoutersAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "NSC06");
    ...
    const postnew = async (data: any) => {
      const result = await postNewRouter(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/network/routers?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeNetwork, dispatch, "NSC06");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Router`} />
