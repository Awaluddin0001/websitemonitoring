...
import {
  listBrandLightReducer,
  initialStateListBrandLight,
...
import { LightBrand } from "@/types/lightingTypes";
...
export default function ListBrandLighting() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandLighting(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<LightBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/lighting?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteLighting(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Lighting`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/lighting`)
            }
