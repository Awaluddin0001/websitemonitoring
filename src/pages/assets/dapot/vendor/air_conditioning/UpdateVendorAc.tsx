...
import {
  getOneVendorAirconditioning,
  updateVendorAirconditioning,
} from "@/services/air_conditioning/dapotAirConditioning";
export default function UpdateVendorAirConditioning() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateVendorAirconditioning(
        company,
        company_user_name,
        number_phone,
        searchParams.get("id")!,
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneVendorAirconditioning(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
