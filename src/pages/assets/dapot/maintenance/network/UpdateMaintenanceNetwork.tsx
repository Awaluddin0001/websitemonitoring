...
import {
  getOneMaintenanceNetwork,
  updateMaintenanceNetwork,
...
export default function UpdateMaintenanceNetwork() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceNetwork(data, dispatch);
      if (result.success) {
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceNetwork(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Network`} />
