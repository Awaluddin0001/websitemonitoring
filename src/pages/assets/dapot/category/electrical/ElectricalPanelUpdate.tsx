...
import { getPanel, updatePanel } from "@/services/electrical/dapotPanels";
...
export default function ElectricalPanelUpdate() {
  const navigate = useNavigate();
  const [searchParams, _] = useSearchParams();
  const [state, dispatch] = useReducer(updatePanelReducer, initialPanelState);
  const {
    asset_id,
    ne_id,
    site_id,
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "ESC005");
    ...
    formData.append("function_category", function_category);
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/electrical/panel?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getPanel(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_PANEL",
          payload: {
            ...
            function_category: data.function_category,
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Function Category</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={function_category || ""}
                    onChange={(e) =>
                      dispatch({
