...
import {
  getOneMaintenanceElectrical,
  updateMaintenanceElectrical,
} from "@/services/electrical/dapotElectrical";
import React, { useEffect, useReducer } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  initialStateUpdateElectricalMaintenance,
  updateElectricalMaintenanceReducer,
} from "src/reducers/electricalReducer";
export default function UpdateMaintenanceElectrical() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceElectrical(data, dispatch);
      if (result.success) {
        navigate(`/main/assets/datapotensi/maintenance/list/electrical?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceElectrical(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Electrical`} />
