...
import {
  initialStateListAirdevice,
  listAirdeviceReducer,
} from "src/reducers/airConditioningReducer";
import { AirDevice } from "@/types/airConditioningTypes";
import {
  deleteAir,
  exportAirsCsv,
  exportAirsXlsx,
  getAirs,
} from "@/services/air_conditioning/dapotAir";
export default function AirconditioningAir() {
  ...
  useEffect(() => {
    const fetchAirs = async () => {
      try {
        ...
        const data = await getAirs(page, dispatch, globalFilter, nopage);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_AIRDEVICES", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch airs",
        });
      }
    };
  ...
  const columns: ColumnDef<AirDevice>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Air Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/airconditioning/air?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "air_flow", header: "Arah Angin" },
        { accessorKey: "speed", header: "Kecepatan" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/airconditioning/air?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteAir(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportAirsCsv}
            exportXlsx={exportAirsXlsx}
          />
