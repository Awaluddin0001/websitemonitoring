...
import {
  initialStateConveyance,
  updateConveyanceReducer,
...
export default function ConveyanceConveyanceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/conveyance/conveyance?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getConveyance(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_CONVEYANCE",
          payload: {
        ...
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch conveyance",
        });
      }
    };
    ...
    fetchBrand(getBrandConveyance, dispatch);
    fetchVendor(getVendorConveyance, dispatch);
    fetchTypeNonSc(getTypeConveyance, dispatch);
    fetchMaintenance(getMaintenanceConveyance, dispatch);
  }, []);
