...
import { safetyListButtons } from "@/routes/dapotCategory";
import { Safety } from "@/types/safetyTypes";
import { renderPagination } from "@/components/table/RenderPagination";
import {
  initialStateSafetyAllList,
  updateSafetyAllReducer,
} from "src/reducers/safetyReducer";
import {
  deleteSafetydevice,
  exportSafetydeviceCsv,
  exportSafetydeviceXlsx,
  getSafetydevices,
} from "@/services/safety/dapotSafety";

export default function SafetyDevice() {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      try {
        ...
        const data = await getSafetydevices(
          page,
          dispatch,
          globalFilter,
          nopage
        ...
        dispatch({ type: "SET_SAFETYS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
  ...
  const columns: ColumnDef<Safety>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Device Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/safety/safety?id=${row.original.asset_id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/safety/safety?id=${row.original.asset_id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteSafetydevice(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportSafetydeviceCsv}
            exportXlsx={exportSafetydeviceXlsx}
          />
          <div className={styles.tableSeal}>
            <div className={styles.tableWrapper}>
              <DapotButtonsCategory
                listpages={safetyListButtons}
                subCategory="safety"
              />
              {safetys.length > 0 ? (
