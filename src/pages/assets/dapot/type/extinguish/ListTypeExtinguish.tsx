...
import {
  initialStateListTypeExtinguish,
  listTypeExtinguishReducer,
} from "src/reducers/extinguishReducer";

import {
  deleteTypeExtinguish,
  getTypeExtinguish,
...
import { ExtinguishBrand } from "@/types/extinguishTypes";
...
export default function ListTypeExtinguish() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        const data = await getTypeExtinguish(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<ExtinguishBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/type/update/extinguish?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTypeExtinguish(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Type Extinguish`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/type/add/extinguish`)
            }
