...
import { postTypeConveyance } from "@/services/conveyance/dapotConveyance";
...
export default function NewTypeConveyance() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (name: string) => {
      const result = await postTypeConveyance(name, dispatch);
      if (result.success) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Type Conveyance`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Input Data Type Untuk Conveyance`} />
