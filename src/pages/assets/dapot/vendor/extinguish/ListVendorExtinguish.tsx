...
import {
  initialStateListVendorExtinguish,
  listVendorExtinguishReducer,
} from "src/reducers/extinguishReducer";
import {
  deleteVendorExtinguish,
  getVendorExtinguish,
} from "@/services/extinguish/dapotExtinguish";
import { ExtinguishVendor } from "@/types/extinguishTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorExtinguish() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorExtinguish(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<ExtinguishVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/extinguish?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorExtinguish(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
