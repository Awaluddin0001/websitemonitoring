...
import { pumpListButtons } from "@/routes/dapotCategory";
import { Device } from "@/types/pumpTypes";
import { renderPagination } from "@/components/table/RenderPagination";
import {
  initialStatePumpAllList,
  updatePumpAllReducer,
} from "src/reducers/pumpReducer";
import {
  deletePumpdevice,
  exportPumpdeviceCsv,
  exportPumpdeviceXlsx,
  getPumpdevices,
} from "@/services/pump/dapotPump";
export default function PumpDevice() {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      try {
        ...
        const data = await getPumpdevices(page, dispatch, globalFilter, nopage);
        console.log(data);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_PUMPS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/pump/pump?id=${row.original.asset_id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "water_flow", header: "Water_flow" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/pump/pump?id=${row.original.asset_id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deletePumpdevice(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportPumpdeviceCsv}
            exportXlsx={exportPumpdeviceXlsx}
          />
          <div className={styles.tableSeal}>
            <div className={styles.tableWrapper}>
              <DapotButtonsCategory
                listpages={pumpListButtons}
                subCategory="pump"
              />
              {pumps.length > 0 ? (
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index === pumps.length - 1 && ind === 0
                                      ? "10px"
                                      : undefined,
                                  borderBottomRightRadius:
                                    row.index === pumps.length - 1 &&
