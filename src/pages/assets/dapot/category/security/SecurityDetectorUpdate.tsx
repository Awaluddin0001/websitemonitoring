...
import { getDetector, updateDetector } from "@/services/security/dapotDetector";
...
export default function SecurityDetectorUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "SSC05");
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/security/detector?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getCctvs = async () => {
      try {
        const data = await getDetector(searchParams.get("id"), dispatch);
        console.log(data);
        dispatch({
          type: "GET_OSC",
          payload: {
    ...
    fetchType(getTypeSecurity, dispatch, "SSC05");
  }, []);
