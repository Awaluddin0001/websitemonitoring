...
import {
  initialStateListBrandFinishingBuilding,
  listBrandFinishingBuildingReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  deleteFinishingbuilding,
  getBrandFinishingbuilding,
} from "@/services/finishing_building/dapotFinishingBuildings";
import { FinishingBuildingBrand } from "@/types/finishingBuildingTypes";

export default function ListBrandFinishing() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandFinishingbuilding(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<FinishingBuildingBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/buildingfinishing?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteFinishingbuilding(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Building Finishing`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/buildingfinishing`)
            }
