...
import {
  ...
  exportMebelsXlsx,
  getMebels,
} from "@/services/finishing_building/dapotMebel";
import { finishingListButtons } from "@/routes/dapotCategory";
export default function FinishingMebel() {
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/buildingfinishing/mebel?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/buildingfinishing/mebel?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMebel(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportMebelsCsv}
            exportXlsx={exportMebelsXlsx}
          />
