...
import {
  getOneTypeAirconditioning,
  getSubCategoriesAirconditioning,
  updateTypeAirconditioning,
...
export default function UpdateTypeAirConditioning() {
  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneTypeAirconditioning(
          dispatch,
          searchParams.get("id")
        );
        console.log(result);
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateTypeAirconditioning(
        name,
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Type Air Conditioning`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Update Data Type Untuk Air Conditioning`} />
