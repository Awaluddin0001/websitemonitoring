...
import { postNewWindow } from "@/services/finishing_building/dapotWindow";
...
export default function FinishingWindowAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "BSC06");
    ...
    const postnew = async (data: any) => {
      const result = await postNewWindow(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/buildingfinishing/window?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeFinishingbuilding, dispatch, "BSC06");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk window`} />
