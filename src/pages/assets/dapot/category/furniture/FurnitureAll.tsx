...
import { getFurnitures } from "@/services/furniture/dapotFurniture";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
export default function FurnitureAll() {
  ...
  useEffect(() => {
    const fetchFurnitures = async () => {
      try {
        ...
        const data = await getFurnitures(page, dispatch, globalFilter, nopage);
        ...
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch furniture",
        });
      }
    };
  ...
  const columns: ColumnDef<Furniture>[] = [
    { accessorKey: "asset_id", header: "Furniture ID" },
    ...
    { accessorKey: "room_name", header: "Ruangan" },
    {
      ...
      cell: ({ row }) => (
        <Link
          to={`/main/assets/datapotensi/detail/furniture/furniture?id=${row.original.device_id}`}
          style={{
            color: "#000",
            fontSize: "1.8rem",
          }}
