...
import {
  listBrandFurnitureReducer,
  initialStateListBrandFurniture,
...
import { FurnitureBrand } from "@/types/furnitureTypes";
...
export default function ListBrandFurniture() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandFurniture(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<FurnitureBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/furniture?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteFurniture(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Furniture`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/furniture`)
            }
