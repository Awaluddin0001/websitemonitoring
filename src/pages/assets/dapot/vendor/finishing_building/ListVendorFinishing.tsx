...
import {
  initialStateListVendorFinishingBuilding,
  listVendorFinishingBuildingReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  deleteVendorFinishingbuilding,
  getVendorFinishingbuilding,
} from "@/services/finishing_building/dapotFinishingBuildings";
import { FinishingBuildingVendor } from "@/types/finishingBuildingTypes";

export default function ListVendorFinishing() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorFinishingbuilding(
          page,
          dispatch,
          globalFilter,
          nopage
        ...
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch vendor",
        });
      }
    };
    fetchVendors();
  ...
  const columns: ColumnDef<FinishingBuildingVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/buildingfinishing?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorFinishingbuilding(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Vendor Building Finishing `} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/vendor/add/buildingfinishing`)
            }
