...
import {
  listMaintenanceFluidReducer,
  initialStateListMaintenanceFluid,
} from "src/reducers/fluidReducer";

import {
  deleteMaintenanceFluid,
  getMaintenanceFluid,
...
import { FluidMaintenance } from "@/types/fluidTypes";
...
export default function ListMaintenanceFluid() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenanceFluid(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<FluidMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/fluid?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenanceFluid(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Fluid`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/maintenance/add/fluid`)
            }
