...
import {
  initialStateListTypeConveyance,
  listTypeConveyanceReducer,
} from "src/reducers/conveyanceReducer";

import {
  deleteTypeConveyance,
  getTypeConveyance,
...
export default function ListTypeConveyance() {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
      try {
        ...
        const data = await getTypeConveyance(
          page,
          dispatch,
          globalFilter,
          nopage
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/type/update/conveyance?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteTypeConveyance(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Type Conveyance`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/type/add/conveyance`)
            }
