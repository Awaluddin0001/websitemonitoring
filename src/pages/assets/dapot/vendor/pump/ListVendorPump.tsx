...
import {
  initialStateListVendorPump,
  listVendorPumpReducer,
} from "src/reducers/pumpReducer";
import { deleteVendorPump, getVendorPump } from "@/services/pump/dapotPump";
import { PumpVendor } from "@/types/pumpTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorPump() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorPump(page, dispatch, globalFilter, nopage);
  ...
  const columns: ColumnDef<PumpVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/pump?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorPump(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
