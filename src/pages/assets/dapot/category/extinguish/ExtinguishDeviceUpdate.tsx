...
import {
  getBrandExtinguish,
  getExtinguishdevice,
  getMaintenanceExtinguish,
  getTypeExtinguish,
  getVendorExtinguish,
  updateExtinguishdevice,
...
export default function ExtinguishDeviceUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("system_wiring", system_wiring);
    formData.append("extinguish_agent", extinguish_agent);
    ...
    const postnew = async (data: any) => {
      const result = await updateExtinguishdevice(
        data,
        dispatch,
        asset_id,
        asset_id
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/extinguish/extinguish?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getExtinguishdevice(
          searchParams.get("id"),
          dispatch
        );
        dispatch({
          ...
          payload: {
            ...
            name: data.name,
            system_wiring: data.system_wiring,
            extinguish_agent: data.extinguish_agent,
            weight: data.weight,
    ...
    fetchBrand(getBrandExtinguish, dispatch);
    fetchVendor(getVendorExtinguish, dispatch);
    fetchTypeNonSc(getTypeExtinguish, dispatch);
    fetchMaintenance(getMaintenanceExtinguish, dispatch);
  }, []);
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>System Wiring</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={system_wiring || ""}
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Extinguish Agent</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={extinguish_agent || ""}
                    onChange={(e) =>
                      dispatch({
                        type: "SET_EXTINGUISH_AGENT",
                        payload: e.target.value,
                      })
                    }
                  />
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Berat</p>
                  <input
                    type="number"
                    className={styles.inputAsset}
                    value={weight || ""}
