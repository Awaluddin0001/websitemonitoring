...
import {
  listBrandExtinguishReducer,
  initialStateListBrandExtinguish,
...
export default function ListBrandExtinguish() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandExtinguish(
          page,
          dispatch,
          globalFilter,
          nopage
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/extinguish?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteExtinguish(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Extinguish`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/extinguish`)
            }
