...
import {
  ...
  exportButtonsXlsx,
  getButtons,
} from "@/services/security/dapotButton";
import {
  initialStateListOsc,
  listOscReducer,
...
export default function SecurityButton() {
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/security/button?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/security/button?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteButton(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportButtonsCsv}
            exportXlsx={exportButtonsXlsx}
          />
