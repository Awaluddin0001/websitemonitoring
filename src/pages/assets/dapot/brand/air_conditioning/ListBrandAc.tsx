...
import {
  initialStateListBrandAirconditioning,
  listBrandAirconditioningReducer,
} from "src/reducers/airConditioningReducer";
import {
  deleteAirconditioning,
  getBrandAirconditioning,
} from "@/services/air_conditioning/dapotAirConditioning";
import { AirconditioningBrand } from "@/types/airConditioningTypes";

export default function ListBrandAircondtioning() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandAirconditioning(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<AirconditioningBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/airconditioning?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteAirconditioning(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Air conditioning`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/airconditioning`)
            }
