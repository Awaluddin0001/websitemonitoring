...
import {
  initialStateListVendorConveyance,
  listVendorConveyanceReducer,
} from "src/reducers/conveyanceReducer";
import {
  deleteVendorConveyance,
  getVendorConveyance,
} from "@/services/conveyance/dapotConveyance";
import { ConveyanceVendor } from "@/types/conveyanceTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorConveyance() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorConveyance(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<ConveyanceVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/conveyance?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorConveyance(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
