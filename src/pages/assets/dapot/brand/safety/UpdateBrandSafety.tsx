...
import {
  initialStateUpdateSafetyBrand,
  updateSafetyBrandReducer,
} from "src/reducers/safetyReducer";
export default function UpdateBrandSafety() {
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneBrandSafety(
          dispatch,
          searchParams.get("id")
        );
        console.log(result);
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (name: any) => {
      const result = await updateBrandSafety(
        name,
        searchParams.get("id")!,
        dispatch
      );
      if (result.success) {
        navigate(`/main/assets/datapotensi/brand/list/safety?page=1`);
      }
    };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Brand Safety`} />
