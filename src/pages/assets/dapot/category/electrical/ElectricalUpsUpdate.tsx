...
import { getUps, updateUps } from "@/services/electrical/dapotUps";
...
export default function ElectricalUpsUpdate() {
  const navigate = useNavigate();
  const [searchParams, _] = useSearchParams();
  const [state, dispatch] = useReducer(updateUpsReducer, initialUpsState);
  const {
    asset_id,
    ne_id,
    site_id,
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "ESC008");
    ...
    formData.append("type_modular", type_modular);
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/electrical/ups?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getRecti = async () => {
      try {
        const data = await getUps(searchParams.get("id"), dispatch);
        dispatch({
          type: "GET_UPS",
          payload: {
            ...
            type_modular: data.type_modular,
          ...
          <div className={styles.sectionInput}>
            <div className={styles.posisiInput}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.containerInput}>
                  <p className={styles.textTitleInput}>Type Modular</p>
                  <input
                    type="text"
                    className={styles.inputAsset}
                    value={type_modular || ""}
