...
import { extinguishListButtons } from "@/routes/dapotCategory";
import { Device } from "@/types/extinguishTypes";
import { renderPagination } from "@/components/table/RenderPagination";
import {
  initialStateExtinguishAllList,
  updateExtinguishAllReducer,
} from "src/reducers/extinguishReducer";
import {
  deleteExtinguishdevice,
  exportExtinguishdeviceCsv,
  exportExtinguishdeviceXlsx,
  getExtinguishdevices,
} from "@/services/extinguish/dapotExtinguish";
export default function ExtinguishDevice() {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      try {
        ...
        const data = await getExtinguishdevices(
          page,
          dispatch,
          globalFilter,
          nopage
        ...
        dispatch({ type: "SET_EXTINGUISHS", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/extinguish/extinguish?id=${row.original.asset_id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "system_wiring", header: "System_wiring" },
        { accessorKey: "extinguish_agent", header: "Extinguish_agent" },
        { accessorKey: "capacity", header: "Capacity" },
        { accessorKey: "weight", header: "Berat" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/extinguish/extinguish?id=${row.original.asset_id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteExtinguishdevice(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportExtinguishdeviceCsv}
            exportXlsx={exportExtinguishdeviceXlsx}
          />
          <div className={styles.tableSeal}>
            <div className={styles.tableWrapper}>
              <DapotButtonsCategory
                listpages={extinguishListButtons}
                subCategory="extinguish"
              />
              {extinguishs.length > 0 ? (
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index === extinguishs.length - 1 &&
                                    ind === 0
                                      ? "10px"
                                      : undefined,
