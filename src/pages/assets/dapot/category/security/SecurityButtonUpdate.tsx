...
import { getButton, updateButton } from "@/services/security/dapotButton";
...
export default function SecurityButtonUpdate() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "SSC03");
    ...
    const postnew = async (data: any) => {
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/security/button?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getCctvs = async () => {
      try {
        const data = await getButton(searchParams.get("id"), dispatch);
        console.log(data);
        dispatch({
          type: "GET_OSC",
          payload: {
    ...
    fetchType(getTypeSecurity, dispatch, "SSC03");
  }, []);
