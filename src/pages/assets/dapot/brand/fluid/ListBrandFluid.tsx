...
import {
  listBrandFluidReducer,
  initialStateListBrandFluid,
} from "src/reducers/fluidReducer";

import { deleteFluid, getBrandFluid } from "@/services/fluid/dapotFluid";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
export default function ListBrandFluid() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandFluid(page, dispatch, globalFilter, nopage);
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/fluid?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteFluid(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Fluid`} />
          <div
            onClick={() => navigate(`/main/assets/datapotensi/brand/add/fluid`)}
