...
import {
  listMaintenanceExtinguishReducer,
  initialStateListMaintenanceExtinguish,
} from "src/reducers/extinguishReducer";

import {
  deleteMaintenanceExtinguish,
...
import { ExtinguishMaintenance } from "@/types/extinguishTypes";
...
export default function ListMaintenanceExtinguish() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenanceExtinguish(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<ExtinguishMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/extinguish?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenanceExtinguish(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Extinguish`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/maintenance/add/extinguish`)
            }
