...
import {
  listBrandSecurityReducer,
  initialStateListBrandSecurity,
...
export default function ListBrandSecurity() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandSecurity(
          page,
          dispatch,
          globalFilter,
          nopage
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/security?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteSecurity(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Security`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/brand/add/security`)
            }
