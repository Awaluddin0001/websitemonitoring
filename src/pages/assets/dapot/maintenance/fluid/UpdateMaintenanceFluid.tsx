...
import {
  getOneMaintenanceFluid,
  updateMaintenanceFluid,
} from "@/services/fluid/dapotFluid";
import React, { useEffect, useReducer } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  initialStateUpdateFluidMaintenance,
  updateFluidMaintenanceReducer,
} from "src/reducers/fluidReducer";
export default function UpdateMaintenanceFluid() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceFluid(data, dispatch);
      if (result.success) {
        navigate(`/main/assets/datapotensi/maintenance/list/fluid?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceFluid(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Fluid`} />
