...
import { postNewPatchpanel } from "@/services/network/dapotPatchPanels";
...
export default function NetworkPatchPanelsAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "NSC04");
    ...
    const postnew = async (data: any) => {
      const result = await postNewPatchpanel(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/network/patch_panels?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeNetwork, dispatch, "NSC04");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Patch panels`} />
