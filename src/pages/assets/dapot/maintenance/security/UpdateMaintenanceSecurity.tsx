...
import {
  getOneMaintenanceSecurity,
  updateMaintenanceSecurity,
} from "@/services/security/dapotSecurity";
import React, { useEffect, useReducer } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  initialStateUpdateSecurityMaintenance,
  updateSecurityMaintenanceReducer,
} from "src/reducers/securityReducer";
export default function UpdateMaintenanceSecurity() {
  ...
  const submitHandler = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (data: any) => {
      const result = await updateMaintenanceSecurity(data, dispatch);
      if (result.success) {
        navigate(`/main/assets/datapotensi/maintenance/list/security?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneMaintenanceSecurity(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Update Data Maintenance Security`} />
