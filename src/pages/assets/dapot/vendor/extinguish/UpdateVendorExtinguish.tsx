...
import {
  getOneVendorExtinguish,
  updateVendorExtinguish,
...
export default function UpdateVendorExtinguish() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateVendorExtinguish(
        company,
        company_user_name,
        number_phone,
        searchParams.get("id")!,
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneVendorExtinguish(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
