...
import {
  initialStateUpdateFinishingBuildingVendor,
  updateFinishingBuildingVendorReducer,
} from "src/reducers/finishingBuildingReducer";
import {
  getOneVendorFinishingbuilding,
  updateVendorFinishingbuilding,
} from "@/services/finishing_building/dapotFinishingBuildings";
export default function UpdateVendorFinishing() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateVendorFinishingbuilding(
        company,
        company_user_name,
        number_phone,
        searchParams.get("id")!,
      ...
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/vendor/list/buildingfinishing?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneVendorFinishingbuilding(
          dispatch,
          searchParams.get("id")
        );
        if (result) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Brand Finishing Building`} />
          <ErrorFetch message={isError} />
        </>
      ) : (
        <>
          <HeadPage title={`Input Data Vendor Untuk Finishing Building`} />
