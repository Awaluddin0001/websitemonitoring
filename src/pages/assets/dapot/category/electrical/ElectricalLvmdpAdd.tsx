...
import { postNewLvmdp } from "@/services/electrical/dapotLvmdp";
interface Options {
  value: string;
  label: string;
}
...
export default function ElectricalLvmdpAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await postNewLvmdp(data, dispatch);
      if (result.success) {
        navigate(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Lvmdp`} />
