...
import { postNewStorage } from "@/services/network/dapotStorage";
...
export default function NetworkStorageAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "NSC07");
    ...
    const postnew = async (data: any) => {
      const result = await postNewStorage(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/network/storage?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeNetwork, dispatch, "NSC07");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Storage`} />
