...
import { postNewCeiling } from "@/services/finishing_building/dapotCeiling";
...
export default function FinishingCeilingAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    formData.append("sub_category_id", "BSC01");
    ...
    const postnew = async (data: any) => {
      const result = await postNewCeiling(data, dispatch);
      if (result.success) {
        navigate(
          `/main/assets/datapotensi/category/list/buildingfinishing/ceiling?page=1`
        );
      }
    };
  ...
  useEffect(() => {
    ...
    fetchType(getTypeFinishingbuilding, dispatch, "BSC01");
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk ceiling`} />
