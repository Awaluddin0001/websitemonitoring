...
import { Cubicle } from "@/types/electricalTypes";
import {
  deleteCubicle,
  exportCubiclesCsv,
  exportCubiclesXlsx,
  getCubicles,
} from "@/services/electrical/dapotCubicle";
...
export default function ElectricalCubicle() {
  ...
  useEffect(() => {
    const fetchCubicle = async () => {
      try {
        const page = searchParams.get("page") || "1";
        const nopage = globalFilter ? "no" : undefined;
        const data = await getCubicles(page, dispatch, globalFilter, nopage);
        console.log(data);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_CUBICLES", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch cubicles",
        });
      }
    };
  ...
  const columns: ColumnDef<Cubicle>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Cubicle Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/electrical/cubicle?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        {
          accessorKey: "load_break",
          header: "Load Break",
        },
        {
          accessorKey: "breaker_count",
          header: "Breaker Count",
        },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/electrical/cubicle?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteCubicle(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportCubiclesCsv}
            exportXlsx={exportCubiclesXlsx}
          />
                            ...
                            <td
                              ...
                              style={{
                                ...
                                borderBottomLeftRadius:
                                  row.index === cubicles.length - 1 && ind === 0
                                    ? "10px"
                                    : undefined,
                                borderBottomRightRadius:
                                  row.index === cubicles.length - 1 &&
