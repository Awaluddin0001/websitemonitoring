...
import { Device } from "@/types/fluidTypes";
...
import {
  deleteFluiddevice,
  exportFluiddeviceCsv,
  exportFluiddeviceXlsx,
  getFluiddevices,
} from "@/services/fluid/dapotFluid";
export default function FluidDevice() {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      try {
        ...
        const data = await getFluiddevices(
          page,
          dispatch,
          globalFilter,
          nopage
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/fluid/fluid?id=${row.original.asset_id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "fluid", header: "Fluid" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/fluid/fluid?id=${row.original.asset_id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteFluiddevice(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportFluiddeviceCsv}
            exportXlsx={exportFluiddeviceXlsx}
          />
