...
import { getOneVendorPump, updateVendorPump } from "@/services/pump/dapotPump";
import React, { useEffect, useReducer } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  initialStateUpdatePumpVendor,
  updatePumpVendorReducer,
} from "src/reducers/pumpReducer";
export default function UpdateVendorPump() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const postnew = async (
      ...
      const result = await updateVendorPump(
        company,
        company_user_name,
        number_phone,
        searchParams.get("id")!,
      ...
      if (result.success) {
        navigate(`/main/assets/datapotensi/vendor/list/pump?page=1`);
      }
    };
  ...
  useEffect(() => {
    const getdata = async () => {
      if (searchParams.get("id")) {
        const result = await getOneVendorPump(dispatch, searchParams.get("id"));
        if (result) {
          dispatch({
            type: "SET_VENDOR",
            payload: result.data,
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Input Data Vendor Untuk Pump`} />
