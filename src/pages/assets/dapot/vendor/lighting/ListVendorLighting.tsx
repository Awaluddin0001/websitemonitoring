...
import {
  initialStateListVendorLight,
  listVendorLightReducer,
} from "src/reducers/lightingReducer";
import {
  deleteVendorLighting,
  getVendorLighting,
} from "@/services/lighting/dapotLighting";
import { LightVendor } from "@/types/lightingTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorLighting() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorLighting(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<LightVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/lighting?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorLighting(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
    ...
    <>
      {isLoading ? (
        ...
        <>
          ...
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/vendor/add/lighting`)
            }
