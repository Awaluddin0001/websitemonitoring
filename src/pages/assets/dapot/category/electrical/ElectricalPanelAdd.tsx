...
import { postNewPanel } from "@/services/electrical/dapotPanels";
interface Options {
  value: string;
  label: string;
}
...
export default function ElectricalPanelAdd() {
  ...
  const handleSubmit = (e: any) => {
    ...
    const postnew = async (data: any) => {
      const result = await postNewPanel(data, dispatch);
      if (result.success) {
        navigate(
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`Aset Baru Untuk Panel`} />
