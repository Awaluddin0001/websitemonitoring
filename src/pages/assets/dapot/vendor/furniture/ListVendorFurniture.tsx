...
import {
  initialStateListVendorFurniture,
  listVendorFurnitureReducer,
} from "src/reducers/furnitureReducer";
import {
  deleteVendorFurniture,
  getVendorFurniture,
} from "@/services/furniture/dapotFurniture";
import { FurnitureVendor } from "@/types/furnitureTypes";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ListVendorFurniture() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const data = await getVendorFurniture(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<FurnitureVendor>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/vendor/update/furniture?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteVendorFurniture(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Vendor Furniture `} />
          <div
            onClick={() =>
