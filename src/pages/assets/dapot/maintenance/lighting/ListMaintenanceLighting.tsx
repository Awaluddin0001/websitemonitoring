...
import {
  listMaintenanceLightReducer,
  initialStateListMaintenanceLight,
} from "src/reducers/lightingReducer";

import {
  deleteMaintenanceLighting,
...
import { LightMaintenance } from "@/types/lightingTypes";
...
export default function ListMaintenanceLighting() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenanceLighting(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<LightMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/lighting?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenanceLighting(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Lighting`} />
          <div
            onClick={() =>
              navigate(`/main/assets/datapotensi/maintenance/add/lighting`)
            }
