...
import {
  listBrandPumpReducer,
  initialStateListBrandPump,
} from "src/reducers/pumpReducer";

import { deletePump, getBrandPump } from "@/services/pump/dapotPump";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
import { PumpBrand } from "@/types/pumpTypes";
...
export default function ListBrandPump() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getBrandPump(page, dispatch, globalFilter, nopage);
  ...
  const columns: ColumnDef<PumpBrand>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/brand/update/pump?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deletePump(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Brand Pump`} />
          <div
            onClick={() => navigate(`/main/assets/datapotensi/brand/add/pump`)}
