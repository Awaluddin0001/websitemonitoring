...
import {
  ...
  exportCeilingsXlsx,
  getCeilings,
} from "@/services/finishing_building/dapotCeiling";
import { finishingListButtons } from "@/routes/dapotCategory";
export default function FinishingCeiling() {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const [state, dispatch] = useReducer(listCat1Reducer, initialStateListCat1);
  const {
  ...
  useEffect(() => {
    const fetchCeilings = async () => {
      try {
        ...
        const data = await getCeilings(page, dispatch, globalFilter, nopage);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_FINISHINGBUILDINGCAT1S", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
          payload: "Failed to fetch ceiling",
        });
      }
    };
    ...
    ? [
        {
          ...
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/buildingfinishing/ceiling?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/buildingfinishing/ceiling?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteCeiling(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportCeilingsCsv}
            exportXlsx={exportCeilingsXlsx}
          />
          <div className={styles.tableSeal}>
            <div className={styles.tableWrapper}>
              <DapotButtonsCategory
                ...
                subCategory="buildingfinishing"
              />
              {finishingbuildingcat1s.length > 0 ? (
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index ===
                                      finishingbuildingcat1s.length - 1 &&
                                    ind === 0
                                      ? "10px"
                                      : undefined,
