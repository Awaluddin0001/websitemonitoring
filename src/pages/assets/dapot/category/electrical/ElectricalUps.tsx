...
import { Ups } from "@/types/electricalTypes";
import {
  deleteUps,
  exportUpssCsv,
  exportUpssXlsx,
  getUpses,
} from "@/services/electrical/dapotUps";
import { renderPagination } from "@/components/table/RenderPagination";

export default function ElectricalUps() {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const [state, dispatch] = useReducer(listUpsReducer, initialStateListUps);
  const {
    upses,
    pagination,
    isLoading,
  ...
  useEffect(() => {
    const fetchRectifiers = async () => {
      try {
        ...
        const data = await getUpses(page, dispatch, globalFilter, nopage);
        console.log(data);
        dispatch({ type: "SET_PAGINATION", payload: data.pagination });
        dispatch({ type: "SET_UPSES", payload: data.data });
      } catch (err) {
        dispatch({
          type: "SET_IS_ERROR",
  ...
  const columns: ColumnDef<Ups>[] = positionColumn
    ? [
        {
          accessorKey: "id",
          header: "Ups Id",
          cell: ({ row }) => (
            <Link
              to={`/main/assets/datapotensi/detail/electrical/ups?id=${row.original.id}`}
              style={{
                color: "#000",
                fontSize: "1.8rem",
              }}
        ...
        { accessorKey: "type_modular", header: "Type Modular" },
          ...
          cell: ({ row }) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  navigate(
                    `/main/assets/datapotensi/category/update/electrical/ups?id=${row.original.id}`
                  )
                }
              >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteUps(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          <HeadPageDapot
            ...
            exportToggle={exportToggle}
            setToggle={dispatch}
            exportCsv={exportUpssCsv}
            exportXlsx={exportUpssXlsx}
          />
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  borderBottomLeftRadius:
                                    row.index === upses.length - 1 && ind === 0
                                      ? "10px"
                                      : undefined,
                                  borderBottomRightRadius:
                                    row.index === upses.length - 1 &&
