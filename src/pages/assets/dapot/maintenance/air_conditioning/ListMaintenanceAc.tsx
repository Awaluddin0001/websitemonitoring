...
import {
  initialStateListMaintenanceAirconditioning,
  listMaintenanceAirconditioningReducer,
} from "src/reducers/airConditioningReducer";
import { AirconditioningMaintenance } from "@/types/airConditioningTypes";
import {
  deleteMaintenanceAirconditioning,
  getMaintenanceAirconditioning,
} from "@/services/air_conditioning/dapotAirConditioning";

export default function ListMaintenanceAir() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        const data = await getMaintenanceAirconditioning(
          page,
          dispatch,
          globalFilter,
          nopage
  ...
  const columns: ColumnDef<AirconditioningMaintenance>[] = [
      ...
      cell: ({ row }) => (
        ...
        >
          <div
            ...
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/update/airconditioning?id=${row.original.id}`
              )
            }
          >
  ...
  const deleteHandle = async (id: string, assetId: string) => {
    const data = await deleteMaintenanceAirconditioning(dispatch, id, assetId);
    if (data.success) {
      navigate(0);
    }
  };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <HeadPage title={`List Maintenance Air Conditioning`} />
          <div
            onClick={() =>
              navigate(
                `/main/assets/datapotensi/maintenance/add/airconditioning`
              )
            }
